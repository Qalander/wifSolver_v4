...
void processingUnifiedUnit(const uint8_t gpuIx, const int deviceId, uint64_t **dev_buffRangeStart, Int rangeStart, const uint32_t expectedChecksum, uint32_t *buffResultManaged, bool *buffIsResultManaged, cudaError_t *gpuStatuses);
cudaError_t executeUnifiedKernel(uint32_t *buffResultManaged, bool *buffIsResultManaged, uint64_t *const dev_buffRangeStart, const uint32_t expectedChecksum);
string expandMaskedWif(const string &input, char replacementChar);
bool tryBuildExpandedWif(const string &input, char replacementChar, string &output, bool recordRepeatedCharacters);
string decodeBase58WifToHex(const string &wif);
void setIntFromWif(const string &wif, Int &target);
string generateRandomBase58String(int length);
vector<unsigned char> buildKeyBuffer(Int &value);
...
{
    string result = input;
    for (int length = 15; length >= 4; --length)
    {
        result = regex_replace(result, regex(string(length, 'X')), string(length, replacementChar));
    }
    return result;
}
...
bool tryBuildExpandedWif(const string &input, char replacementChar, string &output, bool recordRepeatedCharacters)
{
    ...
    {
        if ((asciiArray[ch] > 2) &&
            (ch != static_cast<unsigned char>(alreadyprinted)) &&
            (find(alreadyprintedcharacters.begin(), alreadyprintedcharacters.end(), static_cast<char>(ch)) == alreadyprintedcharacters.end()))
        {
            if (recordRepeatedCharacters)
            {
                alreadyprintedcharacters.push_back(static_cast<char>(ch));
                alreadyprinted = static_cast<char>(ch);
            }
            output = expandMaskedWif(input, replacementChar);
        }
    }
...
string decodeBase58WifToHex(const string &wif)
{
    size_t decodedLength = wif.size() == 52 ? 38 : 37;
    vector<unsigned char> decoded(decodedLength);
    b58decode(decoded.data(), &decodedLength, wif.c_str(), wif.size());

    string hex;
    hex.reserve(decodedLength * 2);
    for (size_t i = 0; i < decodedLength; i++)
    {
        char byteHex[3];
        snprintf(byteHex, sizeof(byteHex), "%.2x", decoded[i]);
        hex += byteHex;
    }
    return hex;
}
...
void setIntFromWif(const string &wif, Int &target)
{
    string hexValue = decodeBase58WifToHex(wif);
    target.SetBase16((char *)hexValue.c_str());
}
...
string generateRandomBase58String(int length)
{
    ...
    {
        result += BASE58_ALPHABET[rand() % 58];
    }
    return result;
}
...
vector<unsigned char> buildKeyBuffer(Int &value)
{
    vector<unsigned char> buffer(dataLen);
    for (int i = 0, d = dataLen - 1; i < dataLen; i++, d--)
    {
        buffer[i] = value.GetByte(d);
    }
    return buffer;
}
...
bool processPrefixWIF()
{
    ...
    if (!tryBuildExpandedWif(WIF, '1', WIFSTART, true))
    {
    ...
    if (!tryBuildExpandedWif(WIF, 'z', WIFEND, true))
    {
        fprintf(stderr, "  ERROR: Could not extract WIFEND from WIF\n\n");
        return false;
    }

    setIntFromWif(WIFSTART, RANGE_START);
    setIntFromWif(WIFSTART, start1);
    setIntFromWif(WIFEND, RANGE_END);
...
cudaError_t processCudaUnified()
{
    printf("  ----------------***processCudaUnified***-----------------\n");
    cudaError_t cudaStatus;
    const int gpuCount = DEVICE_NR == -1 ? nDevices : 1;
    uint64_t *buffStride = new uint64_t[NB64BLOCK];

    const int COLLECTOR_SIZE_MM_PER_GPU = 4 * BLOCK_NUMBER * BLOCK_THREADS;
    const int COLLECTOR_SIZE_MM = COLLECTOR_SIZE_MM_PER_GPU * gpuCount;
    const uint32_t expectedChecksum = IS_CHECKSUM ? CHECKSUM.GetInt32() : 0;
    uint64_t counter = 0;
    uint32_t *buffResultManaged = nullptr;
    bool *buffIsResultManaged = nullptr;
    uint64_t **dev_buffRangeStart = nullptr;
    cudaError_t *gpuStatuses = nullptr;
    ...
    for (int gpuIx = 0; gpuIx < gpuCount; gpuIx++)
    {
        cudaSetDevice(activeDevices[gpuIx]);
    ...
    cudaStatus = cudaMallocManaged(&buffIsResultManaged, gpuCount * sizeof(bool));
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "  cudaMallocManaged failed 'buffIsResultManaged': %s\n", cudaGetErrorString(cudaStatus));
        goto Error;
    }
    for (int gpuIx = 0; gpuIx < gpuCount; gpuIx++)
    {
    ...
    dev_buffRangeStart = new uint64_t *[gpuCount];
    gpuStatuses = new cudaError_t[gpuCount];
    threads = new std::thread[gpuCount];
    for (int gpuIx = 0; gpuIx < gpuCount; gpuIx++)
    {
        dev_buffRangeStart[gpuIx] = nullptr;
        gpuStatuses[gpuIx] = cudaSuccess;
        cudaSetDevice(activeDevices[gpuIx]);
        cudaStatus = cudaMalloc((void **)&dev_buffRangeStart[gpuIx], NB64BLOCK * sizeof(uint64_t));
        if (cudaStatus != cudaSuccess)
        {
            fprintf(stderr, "  %d:: cudaMalloc failed 'dev_buffRangeStart': %s\n", activeDevices[gpuIx], cudaGetErrorString(cudaStatus));
            goto Error;
        }
    }
    ...
    while (!RESULT && RANGE_START.IsLower(&RANGE_END))
    {
        ...
        bool gpuFailure = false;
        for (int gpuIx = 0; gpuIx < gpuCount; gpuIx++)
        {
            threads[gpuIx] = std::thread(processingUnifiedUnit, gpuIx, activeDevices[gpuIx], dev_buffRangeStart, rangeTestStart, expectedChecksum, buffResultManaged, buffIsResultManaged, gpuStatuses);
        }

        for (int gpuIx = 0; gpuIx < gpuCount; gpuIx++)
        {
            threads[gpuIx].join();
            if (gpuStatuses[gpuIx] != cudaSuccess)
            {
                cudaStatus = gpuStatuses[gpuIx];
                gpuFailure = true;
                break;
            }
        }
...
Error:
    if (DEVICE_NR == -1)
    {
        for (int gpuIx = 0; gpuIx < gpuCount; gpuIx++)
        {
            if (dev_buffRangeStart != nullptr && dev_buffRangeStart[gpuIx] != nullptr)
            {
                cudaSetDevice(activeDevices[gpuIx]);
                cudaFree(dev_buffRangeStart[gpuIx]);
            }
        }
    }
    else if (dev_buffRangeStart != nullptr && dev_buffRangeStart[0] != nullptr)
    {
        cudaSetDevice(activeDevices[0]);
        cudaFree(dev_buffRangeStart[0]);
    }
    delete[] dev_buffRangeStart;
    delete[] gpuStatuses;
    delete[] threads;
    if (buffResultManaged != nullptr)
    {
        cudaFree(buffResultManaged);
    }
    if (buffIsResultManaged != nullptr)
    {
...
void processingUnifiedUnit(const uint8_t gpuIx, const int deviceId, uint64_t **dev_buffRangeStart, Int rangeStart, const uint32_t expectedChecksum, uint32_t *buffResultManaged, bool *buffIsResultManaged, cudaError_t *gpuStatuses)
{
    gpuStatuses[gpuIx] = cudaSetDevice(deviceId);
    if (gpuStatuses[gpuIx] != cudaSuccess)
    {
        fprintf(stderr, "  %d:: cudaSetDevice failed: %s\n", deviceId, cudaGetErrorString(gpuStatuses[gpuIx]));
        return;
    }
    ...
    if (gpuIx > 0)
    {
        Int deviceOffset = new Int((uint64_t)gpuIx);
        deviceOffset.Mult(&loopStride);
        tempStart.Add(&deviceOffset);
    }

    uint64_t *tmpBufferStart = new uint64_t[NB64BLOCK];
    __Load(tmpBufferStart, tempStart.bits64);
    gpuStatuses[gpuIx] = cudaMemcpy(dev_buffRangeStart[gpuIx], tmpBufferStart, NB64BLOCK * sizeof(uint64_t), cudaMemcpyHostToDevice);
    delete[] tmpBufferStart;
    if (gpuStatuses[gpuIx] != cudaSuccess)
    {
    ...
    gpuStatuses[gpuIx] = executeUnifiedKernel(
        buffResultManaged + (gpuIx * (4 * BLOCK_NUMBER * BLOCK_THREADS)),
        buffIsResultManaged + gpuIx,
        dev_buffRangeStart[gpuIx],
        expectedChecksum);
    if (gpuStatuses[gpuIx] != cudaSuccess)
    {
...
void printSpeed(double speed)
{
    std::array<char, 53> wif = {};
    vector<unsigned char> buff = buildKeyBuffer(RANGE_START);
    b58encode(wif.data(), &wifLen, buff.data(), dataLen);
    ...
    TurboRotation::spin(turboCtx, std::string(wif.data()));
    ...
    if (part1 != "")
    {
        ...
        if (zapusk > 30)
        {
            srand(time(NULL));
            int N = kusok;
            string pass = generateRandomBase58String(N);
            std::stringstream ss8;
            ss8 << part1.c_str() << pass << part2.c_str();
            string rannd = ss8.str();

            setIntFromWif(rannd, RANGE_START);
            zapusk = 0;
        }
    }
    ...
    if (turbo > 0)
    {
        printf("\r Speed:  [%s] [%s] [S: %s%s] [C: %.3f%%] [T: %s]  ", toTimeStr(t1, timeStr), wif.data(), formatThousands(speedx).c_str(), valx.c_str(), _count, formatThousands(vtotal).c_str());
    }
    else
    {
        if (step2 > 8 || part1 != "")
        ...
        {
            printf("\r  [%s] [%s] [S: %s%s] [C: %.3f%%] [T: %s]  ", toTimeStr(t1, timeStr), wif.data(), formatThousands(speedx).c_str(), valx.c_str(), _count, formatThousands(vtotal).c_str());
        }
    }
    if (step2 > 8)
    {
...
void processCandidate(Int &toTest)
{
    ...
    std::array<char, 53> wif = {};
    vector<unsigned char> buff = buildKeyBuffer(toTest);
    ...
    if (!TARGET_ADDRESS.empty())
    {
        if (TARGET_ADDRESS == address)
        {
            ...
            if (b58encode(wif.data(), &wifLen, buff.data(), dataLen))
            {
                printf("  WIF key    : %s\n", wif.data());
            }
            printf("  ===============================================================================\n");
...
bool checkDevice()
{
    ...
    if (DEVICE_NR == -1)
    {
        ...
        int detectedBlockNumber = 0;
        int detectedBlockThreads = 0;
        for (int i = 0; i < nDevices; i++)
        {
            ...
            cudaStatus = cudaGetDeviceProperties(&props, i);
            if (cudaStatus != cudaSuccess)
            {
                fprintf(stderr, "  Cannot query device %d properties!", i);
                return false;
            }
            ...
            const int deviceBlockNumber = props.multiProcessorCount * 8;
            const int deviceBlockThreads = (props.maxThreadsPerBlock / 8) * 5;
            if (detectedBlockNumber == 0 || deviceBlockNumber < detectedBlockNumber)
            {
                detectedBlockNumber = deviceBlockNumber;
            }
            if (detectedBlockThreads == 0 || deviceBlockThreads < detectedBlockThreads)
            {
                detectedBlockThreads = deviceBlockThreads;
            }
        }

        if (BLOCK_NUMBER == 0)
        {
            BLOCK_NUMBER = detectedBlockNumber;
        }
        if (BLOCK_THREADS == 0)
        {
            BLOCK_THREADS = detectedBlockThreads;
        }
    ...
    activeDevices.push_back(DEVICE_NR);
    cudaDeviceProp props;
...
void showHelp()
{
    ...
    printf("-d               DeviceId or 'all' for multi-GPU (default 0)\n");
...
bool readArgs(int argc, char **argv)
{
    ...
    if (WIF != "")
    {
        ...
        tryBuildExpandedWif(WIF, '1', WIFSTART, false);
    }
    if (WIF != "")
    {

        string expandedWifEnd;
        if (tryBuildExpandedWif(WIF, 'z', expandedWifEnd, false))
        {
            if (expandedWifEnd == WIF)
            {
                if (COMPRESSED)
                {
    ...
    string rangeStartHex = decodeBase58WifToHex(WIFSTART);
    RANGE_START.SetBase16((char *)rangeStartHex.c_str());
    start1.SetBase16((char *)rangeStartHex.c_str());
